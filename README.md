...
* Along the same reasoning as the previous limitation, having hard-coded schema references within objects (functions, procedures, triggers, etc) are also bypassed since clone_schema does not alter the contents within these objects.
...
* Functional dependency order is not supported.  If you have functions that depend on other functions, just make sure that you define them as language as PLPGSQL. Then the order does not matter, but if you define them as language SQL, dependency order is in play and you may get errors.
